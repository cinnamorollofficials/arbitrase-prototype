...
export async function up(queryInterface, Sequelize) {
  await queryInterface.sequelize.transaction(async (transaction) => {
    ...
    if (!exchangeId) {
      ...
      exchangeId = Math.max(Number(maxExchangeRows[0]?.max_id || 0) + 1, 17);
      ...
      await queryInterface.bulkInsert('exchanges', [
        {
          ...
          type: 'CEX',
          is_active: true,
          website_url: 'https://bittime.com',
          logo_url: 'https://bittime.com/favicon.ico',
      ...
      await queryInterface.bulkInsert('exchange_attributes', [
        {
          ...
          attribute_value: 'https://openapi.bittime.com',
          data_type: 'string',
          created_at: now,
          updated_at: now
        }
    ...
    const baseSymbols = [...new Set(bittimeIdrPairs.map((symbol) => symbol.split('_')[0]))];
    ...
    const tokenPairs = bittimeIdrPairs.map((symbol) => {
...
export async function down(queryInterface, Sequelize) {
  await queryInterface.sequelize.transaction(async (transaction) => {
    ...
    const bittime = exchangeRows[0];
    if (!bittime) return;

    await queryInterface.bulkDelete('fees', { exchange_id: bittime.id }, { transaction });
    await queryInterface.bulkDelete('token_pairs', { exchange_id: bittime.id }, { transaction });
    await queryInterface.bulkDelete('exchange_attributes', { exchange_id: bittime.id }, { transaction });
    await queryInterface.bulkDelete('exchanges', { id: bittime.id }, { transaction });
  });
}
